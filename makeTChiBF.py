...
limdir = '/uscms_data/d3/mengleis/CMSSW_Work/CMSSW_7_4_7/src/HiggsAnalysis/CombinedLimit/DataCard/TChiWG_Scan/Limit/'
limdirdata = '/uscms_data/d3/mengleis/CMSSW_Work/CMSSW_7_4_7/src/HiggsAnalysis/CombinedLimit/DataCard/TChiWG_Scan/Limit/'
...
for xbin in range(1, 32): 
    neutralino_mass = np.append(neutralino_mass,400+25*xbin)
for ybin in range(1, 50):
    branch_ratio = np.append(branch_ratio,0.02*ybin)

print(neutralino_mass)
print(branch_ratio)
nbinsx = len(neutralino_mass) - 1
nbinsy = len(branch_ratio) - 1

hExpLimUp = ROOT.TH2D('up', 'up', nbinsx, neutralino_mass, nbinsy, branch_ratio)
hExpLimMid = ROOT.TH2D('mid', 'mid', nbinsx, neutralino_mass, nbinsy, branch_ratio)
hExpLimDown = ROOT.TH2D('down', 'down', nbinsx, neutralino_mass, nbinsy, branch_ratio)
hObsLim = ROOT.TH2D('obs', 'obs', nbinsx, neutralino_mass, nbinsy, branch_ratio)

for i in neutralino_mass:
    for j in branch_ratio:
            if(not os.path.exists(limdir + 'limit_' + str(int(i)) + '_' + str(int(j*100)) +  '.txt')):
                continue
            with open(limdir + 'limit_' + str(int(i)) + '_' + str(int(j*100)) +  '.txt') as fin:
                for line in fin:
            ...
            if(not os.path.exists(limdirdata + 'limit_' + str(int(i)) + '_' + str(int(j*100)) +  '.txt')):
                continue
            with open(limdirdata + 'limit_' + str(int(i)) + '_' + str(int(j*100)) +  '.txt') as findata:
                for line in findata:
...
def ScaleLimit(h, s, scaleByXSec, sigma):
    ...
    for i in range(1, h_new.GetNbinsX()+1):
        for j in range(1, h_new.GetNbinsY()+1):
            ...
            m = h_new.GetXaxis().GetBinLowEdge(i)
...
h_xSecLimit.GetXaxis().SetTitle('M_{#tilde{#chi}^{0/#pm}} (GeV)')
h_xSecLimit.GetYaxis().SetTitle('BF(#tilde{#chi}_{2}^{0}#rightarrow#tilde{#chi}_{1}^{0}  ')
...
c_limit.Draw()
c_limit.SaveAs('/eos/uscms/store/user/tmishra/TChi_Scan.pdf')


#file_out = ROOT.TFile('./T5WG_UnblindLimit.root', 'RECREATE')
file_out = ROOT.TFile('/eos/uscms/store/user/tmishra/TChi_Scan.root', 'RECREATE')
file_out.cd()
h_xSecLimit.Write()
